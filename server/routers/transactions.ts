...
export const transactionsProcedures = {
  ...
  getAllTransactions: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    const caixinha = await getCaixinhaOrThrow(db, ctx.user.id);
    return db.select({ id: transactions.id, participantId: transactions.participantId, type: transactions.type, amount: transactions.amount, balanceBefore: transactions.balanceBefore, balanceAfter: transactions.balanceAfter, month: transactions.month, year: transactions.year, description: transactions.description, createdAt: transactions.createdAt })
      .from(transactions).innerJoin(participants, eq(participants.id, transactions.participantId)).where(eq(participants.caixinhaId, caixinha.id));
  }),
  ...
  getAuditLog: protectedProcedure.input(z.object({ participantId: participantIdSchema.optional(), limit: z.number().int().min(1).max(200).default(50) })).query(async ({ input, ctx }) => {
    ...
    return db.select({ id: auditLog.id, participantId: auditLog.participantId, participantName: auditLog.participantName, action: auditLog.action, month: auditLog.month, year: auditLog.year, amount: auditLog.amount, description: auditLog.description, createdAt: auditLog.createdAt })
      .from(auditLog).innerJoin(participants, eq(participants.id, auditLog.participantId)).where(eq(participants.caixinhaId, caixinha.id)).limit(input.limit);
  }),
  ...
  registerPayment: protectedProcedure.input(z.object({ participantId: participantIdSchema, month: monthSchema, year: z.number().int().min(2020).max(2100), idempotencyKey: z.string().uuid().optional() })).mutation(async ({ input, ctx }) => {
    const db = await getDb();
    const caixinha = await getCaixinhaOrThrow(db, ctx.user.id);
    if (input.idempotencyKey) {
      const [existing] = await db.select().from(transactions).innerJoin(participants, eq(participants.id, transactions.participantId)).where(and(eq(transactions.idempotencyKey, input.idempotencyKey), eq(participants.caixinhaId, caixinha.id))).limit(1);
      if (existing) return { success: true, isLate: false };
    }
    return db.transaction(async (tx) => {
      const [p] = await tx.select().from(participants).where(and(eq(participants.id, input.participantId), eq(participants.isActive, true))).for("update").limit(1);
      if (!p) throw new TRPCError({ code: "NOT_FOUND" });
      const existingPayment = await tx.select().from(monthlyPayments).where(and(eq(monthlyPayments.participantId, input.participantId), eq(monthlyPayments.month, input.month), eq(monthlyPayments.year, input.year))).limit(1);
      if (existingPayment.length > 0 && existingPayment[0].paid === true) throw new TRPCError({ code: "CONFLICT", message: "Mês já pago." });
      ...
      const calc = late ? calcLateMonthlyPayment(currentDebt) : { ...calcMonthlyPayment(currentDebt), isLate: false, lateFee: new Decimal(0), lateInterest: new Decimal(0), totalLateCharge: new Decimal(0) };

      if (existingPayment.length > 0) await tx.update(monthlyPayments).set({ paid: true, paidLate: late, paidAt: new Date() }).where(eq(monthlyPayments.id, existingPayment[0].id));
      else await tx.insert(monthlyPayments).values({ participantId: input.participantId, month: input.month, year: input.year, paid: true, paidLate: late, paidAt: new Date() });

      const description = late ? `Cota R$ 200,00 + Juros R$ ${calc.interest.toFixed(2)} + Multa R$ ${calc.lateFee?.toFixed(2)} + Mora R$ ${calc.lateInterest?.toFixed(2)} (ATRASO)` : `Cota R$ 200,00 + Juros R$ ${calc.interest.toFixed(2)}`;

      try {
        await tx.insert(transactions).values({ participantId: input.participantId, type: "payment", amount: calc.total.toFixed(2), balanceBefore: currentDebt.toFixed(2), balanceAfter: currentDebt.toFixed(2), month: input.month, year: input.year, description, idempotencyKey: input.idempotencyKey });
      } catch (e: any) { if (e?.errno === 1062) return { success: true, isLate: late }; throw e; }

      await tx.insert(auditLog).values({ participantId: input.participantId, participantName: p.name, action: "payment_marked", month: input.month, year: input.year, amount: calc.total.toFixed(2), description });
  ...
  unmarkPayment: protectedProcedure.input(z.object({ paymentId: z.number().int().positive(), participantId: participantIdSchema })).mutation(async ({ input, ctx }) => {
    ...
    return db.transaction(async (tx) => {
      ...
      const [originalTx] = await tx.select().from(transactions).where(and(eq(transactions.participantId, input.participantId), eq(transactions.type, "payment"), eq(transactions.month, payment.month), eq(transactions.year, payment.year))).limit(1);
      
      const currentDebt = new Decimal(p.currentDebt);
      const reversalAmountStr = originalTx ? new Decimal(originalTx.amount).abs().toFixed(2) : calcMonthlyPayment(currentDebt).total.toFixed(2);

      await tx.delete(transactions).where(and(eq(transactions.participantId, input.participantId), eq(transactions.type, "payment"), eq(transactions.month, payment.month), eq(transactions.year, payment.year)));
      await tx.insert(auditLog).values({ participantId: input.participantId, participantName: p.name, action: "payment_unmarked", month: payment.month, year: payment.year, description: `Pagamento de ${payment.month}/${payment.year} desmarcado (estorno gerado)` });
      return { success: true };
    });
  }),
  ...
  resetMonth: protectedProcedure.input(z.object({ month: monthSchema, year: z.number().int().min(2020).max(2100) }).optional()).mutation(async ({ ctx, input }) => {
    ...
    return db.transaction(async (tx) => {
      const payments = await tx.select({ mp: monthlyPayments, participant: participants }).from(monthlyPayments).innerJoin(participants, eq(participants.id, monthlyPayments.participantId)).where(and(eq(participants.caixinhaId, caixinha.id), eq(monthlyPayments.month, month), eq(monthlyPayments.year, year), eq(monthlyPayments.paid, true)));
      ...
      for (const { mp, participant } of payments) {
        ...
        const [originalTx] = await tx.select().from(transactions).where(and(eq(transactions.participantId, mp.participantId), eq(transactions.type, "payment"), eq(transactions.month, month), eq(transactions.year, year))).limit(1);
