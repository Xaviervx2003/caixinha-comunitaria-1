...
export const participantsProcedures = {
  ...
  updateParticipantEmail: protectedProcedure
    .input(z.object({ participantId: participantIdSchema, email: z.string().email().max(320).optional() }))
    .mutation(async ({ input, ctx }) => {
      const db = await getDb();
      const caixinha = await getCaixinhaOrThrow(db, ctx.user.id);
      await getParticipantOrThrow(db, input.participantId, caixinha.id);
  ...
  updateParticipantDebt: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      return db.transaction(async (tx) => {
        ...
        await tx.insert(auditLog).values({ participantId: input.participantId, participantName: p.name, action: "amortization_added", amount: balanceBefore.sub(balanceAfter).abs().toFixed(2), description: `Saldo ajustado manualmente: R$ ${balanceBefore.toFixed(2)} → R$ ${balanceAfter.toFixed(2)}` });
        return { success: true };
      });
    }),
